...
serve(async (req) => {
  ...
  try {
    ...
    if (!transcript && (!messages || messages.length === 0)) {
      return new Response(JSON.stringify({ error: "transcript or messages required" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
...
${conversationText}`
    ...
    if (anamnesisError) {
      console.error("Anamnesis insert error:", anamnesisError);
