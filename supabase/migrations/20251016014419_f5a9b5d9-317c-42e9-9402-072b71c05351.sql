...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'personal_subscriptions') THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    AND tablename = 'personal_subscriptions' 
    AND policyname = 'Personal vê própria assinatura'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'personal_subscriptions' 
    AND policyname = 'Sistema cria assinaturas'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'personal_subscriptions' 
    AND policyname = 'Sistema atualiza assinaturas'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'client_payment_configs' 
    AND policyname = 'Personal gerencia preços de clientes'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Ver assinaturas relacionadas'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Ver próprio histórico'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'commission_settings' 
    AND policyname = 'Admin gerencia comissões'
  ) THEN
